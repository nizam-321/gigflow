...
import { Mail, Lock, User as UserIcon, Sparkles, ArrowRight, CheckCircle } from "lucide-react";
  ...
  return (
    <div className="min-h-screen flex items-center justify-center gradient-bg px-4 py-8">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-20 right-10 w-72 h-72 bg-accent/10 rounded-full blur-3xl float"></div>
        <div className="absolute bottom-20 left-10 w-96 h-96 bg-primary/10 rounded-full blur-3xl float" style={{ animationDelay: '1.5s' }}></div>
      </div>
      ...
      <Card className="w-full max-w-md shadow-2xl border-2 border-primary/10 backdrop-blur-sm bg-card/95 scale-in relative z-10">
        <CardHeader className="flex flex-col items-center space-y-3 pb-6">
          <div className="w-16 h-16 bg-gradient-to-br from-accent to-primary rounded-2xl flex items-center justify-center shadow-lg glow mb-2">
        ...
        <CardContent className="space-y-6">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-2 fade-in-delay-2">
              ...
              <div className="relative">
                <CheckCircle className="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-muted-foreground" />
                <Input
                  id="confirmPassword"
                  type="password"
                  placeholder="••••••••"
            ...
            <Button
              type="submit"
              className="w-full h-12 bg-gradient-to-r from-accent to-primary hover:from-accent/90 hover:to-primary/90 text-white font-semibold shadow-lg btn-hover-lift fade-in-delay-3"
              disabled={loading}
            >
              {loading ? (
